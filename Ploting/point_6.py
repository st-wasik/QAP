...
def point_2(df):
    ...
    plot(time, 'Mean time [ms]', "mean_time-SA-TS.png", OUT)
    plot(checked, 'Mean solutions checked', 'checked_solutions-SA-TS.png', OUT, full=False)
...
def plot(results, y_label, name, out_path, full=True, size=(14, 4), log=True):
    ...
    ax.errorbar(x,
            [item[1]['TABU'][0] for item in results.items()],
            yerr=[item[1]['TABU'][1] for item in results.items()],
            label='Tabu', fmt='-o')
    ax.errorbar(x,
            [item[1]['ANNEALING'][0] for item in results.items()],
            yerr=[item[1]['ANNEALING'][1] for item in results.items()],
            label='Annealing', fmt='-o')
    if full:
        ax.errorbar(x,
...
def get_efficiency(df, optimal_solution):
    ...
    plot(app_err, 'Błąd względny', 'approx_error-SA-TS.png', OUT, log=True)
    plot_without_std(best, 'Najlepszy przypadek', 'best-SA-TS.png', OUT)
...
def quality_in_time(df, eff_dict):
    ...
    plot_without_std(results, 'Efficiency', 'efficiency-SA-TS.png', OUT)
...
def plot_without_std(results, y_label, name, out_path, full=True, size=(14, 4), log=True):
    ...
    ax.plot(x,
           [item[1]['STEEPEST'] for item in results.items()], '-o',
           label='Steepest')
    ax.plot(x,
           [item[1]['GREEDY'] for item in results.items()], '-o',
           label='Greedy')
    ax.plot(x,
           [item[1]['RANDOM'] for item in results.items()], '-o',
           label='Random')
    ax.plot(x,
